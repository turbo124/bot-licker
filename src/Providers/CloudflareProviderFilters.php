 ...
 *
 */
class CloudflareProviderFilters implements ProviderContract
{
    /** @var string $url */
    ...
    private function removeResourceFromFilter($filter, $removable_expression): bool
    {
        ...
        // if(strlen($updated_expression) == 0)
...
echo print_r($expression);
     ...
     * @return bool
     */
    private function deleteFilter(array $filter): bool
    {

        $response =

        Http::withHeaders($this->getHeaders())->delete("{$this->url}zones/{$this->getZone()}/filters?id={$filter['id']}", []);
